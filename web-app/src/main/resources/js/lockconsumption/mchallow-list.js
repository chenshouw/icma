var mchAllowVo = new Object();

function setSearchParam() {

    mchAllowVo.modiDateBegin = $("#modiDateBegin").val();
    mchAllowVo.modiDateEnd = $("#modiDateEndClear").val();

    mchAllowVo.sid = $("#sid").val();
    mchAllowVo.status = $("#status").val();
}
...
var mchallowFrom = $('#mchallowFrom');
mchallowFrom.validate({
    focusInvalid: false,
    ignore: "",
    rules: {
    },
...
$(function () {
    $("#btnSearch").click(function () {
        if (mchallowFrom.valid()) { //验证通过
            $("#mchallowFrom").serialize();
            mchallowFrom.method = "post";
            mchallowFrom.action = "/mchallow/search";
            mchallowFrom.submit();
        }
    });
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.code == 1) {
                    ...
                    var loadUrl = "/mchallow/down-excel-data?tempName=" + tempName + '&fileName=' + fileName;
...
function deletemchallow(ruleId) {
    BootboxExt.confirm("确认删除吗?", function (res) {
        if (res) {
            $.get("/mchallow/todelete", {ruleId: ruleId}, function (data) {
                if (data.result == true) {
